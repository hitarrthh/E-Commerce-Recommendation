...
train_data = pd.read_csv("D:\College\Projects\E-Commerece Recommendation\models\clean_data.csv")
...
@app.route("/main")
def main():
    # Initialize with empty DataFrame for initial page load
...
def recommendations():
    if request.method == 'POST':
        ...
        try:
            ...
            price = [40, 50, 60, 70, 100, 122, 106, 50, 30, 50]
            prices = [random.choice(price) for _ in range(len(content_based_rec))]
            
            # Generate random image URLs if needed
    ...
    # For GET requests, return to main page with empty DataFrame
    return render_template('main.html', 
                         content_based_rec=pd.DataFrame(),
                         truncate=truncate)
